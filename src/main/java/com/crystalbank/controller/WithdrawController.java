...
public class WithdrawController {
    ...
    private void handleWithdraw() {
        ...
        if (amount > SessionManager.getCurrentAccount().getBalance()) {
            showError("Insufficient balance.");
            return;
        }

        if (description.isEmpty()) description = "Cash Withdrawal";

        boolean success = DatabaseManager.withdraw(
            SessionManager.getCurrentAccount().getAccountId(),
            amount, description
        );

        if (success) {
            showSuccess(String.format("Successfully withdrew RM %.2f!", amount));
            amountField.clear();
            ...
            showError("Withdrawal failed. Account may not be active.");
        }
    }
